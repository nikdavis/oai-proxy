...
def client_with_mocks(autouse=True):
    ...
    with patch('os.getenv', return_value=TEST_BOOKS_DIR) as mock_getenv, \
        ...
        try:
            if os.path.exists(TEST_BOOKS_DIR) and (not os.listdir(TEST_BOOKS_DIR) if callable(os.listdir) and not isinstance(os.listdir, MagicMock) or hasattr(os.listdir, '_mock_wraps') and os.listdir._mock_wraps is None else True): # Check if listdir is not a broken mock
                 os.rmdir(TEST_BOOKS_DIR)
            elif os.path.exists(TEST_BOOKS_DIR): # If not empty, log a warning or handle as per test needs
                print(f"Warning: Test directory {TEST_BOOKS_DIR} was not empty after tests.")
        except OSError as e:
            print(f"Warning: OSError during teardown attempting to list or remove {TEST_BOOKS_DIR}: {e}")
...
class TestBangCommandHandlerClientScanDirectory:
    ...
    def test_scan_book_directory_with_txt_files(self, client_with_mocks):
        ...
        def side_effect_isfile(path):
            if path == os.path.join(TEST_BOOKS_DIR, "gatsby.txt"): return True
            if path == os.path.join(TEST_BOOKS_DIR, "moby.txt"): return True
...
class TestBangCommandHandlerClientGetBookDetail:
    async def test_get_book_detail_no_query(self, client_with_mocks):
        client, _, _, _ = client_with_mocks
        client.available_book_files = ["exists.txt"] # Needs some books to pass initial check
    ...
    async def test_get_book_detail_exact_match_success(self, client_with_mocks):
        ...
        with patch('builtins.open', mock_open(read_data=mock_file_content)) as mocked_open_file:
            ...
            mocked_open_file.assert_called_once_with(os.path.join(TEST_BOOKS_DIR, "The Great Gatsby.txt"), 'r', encoding='utf-8')
            assert isinstance(snippet, CommandContextSnippet)
            assert snippet.content["command_query"] == "!book the great gatsby"
            assert f"Content for The Great Gatsby.txt:\n\n{mock_file_content}" == snippet.content["result_text"]
    ...
    async def test_get_book_detail_match_with_txt_in_query(self, client_with_mocks):
        ...
        with patch('builtins.open', mock_open(read_data=mock_file_content)) as mocked_open_file:
            ...
            mocked_open_file.assert_called_once_with(os.path.join(TEST_BOOKS_DIR, "Moby Dick.txt"), 'r', encoding='utf-8')
            assert f"Content for Moby Dick.txt:\n\n{mock_file_content}" == snippet.content["result_text"]
    ...
    async def test_get_book_detail_match_without_txt_in_query(self, client_with_mocks):
        ...
        with patch('builtins.open', mock_open(read_data=mock_file_content)) as mocked_open_file:
            ...
            mocked_open_file.assert_called_once_with(os.path.join(TEST_BOOKS_DIR, "Another Book.txt"), 'r', encoding='utf-8')
            assert f"Content for Another Book.txt:\n\n{mock_file_content}" == snippet.content["result_text"]
    ...
    async def test_get_book_detail_case_insensitive_match(self, client_with_mocks):
        ...
        with patch('builtins.open', mock_open(read_data=mock_file_content)) as mocked_open_file:
            snippet = await client._handle_get_book_detail_command(["uppercase book"]) # Query is lowercase

            mocked_open_file.assert_called_once_with(os.path.join(TEST_BOOKS_DIR, "UPPERCASE BOOK.txt"), 'r', encoding='utf-8')
            assert f"Content for UPPERCASE BOOK.txt:\n\n{mock_file_content}" == snippet.content["result_text"]
    ...
    async def test_get_book_detail_no_match(self, client_with_mocks):
        ...
        assert "Book matching query 'nonexistent book' not found" in snippet.content["result_text"]
    ...
    async def test_get_book_detail_match_file_read_error(self, client_with_mocks):
        ...
        with patch('builtins.open', mock_open()) as mocked_open_file:
            ...
            snippet = await client._handle_get_book_detail_command(["error_book"]) # Exact match for filename part

            mocked_open_file.assert_called_once_with(os.path.join(TEST_BOOKS_DIR, "error_book.txt"), 'r', encoding='utf-8')
            assert "Error reading content for book 'error_book.txt'" in snippet.content["result_text"]
            assert "Cannot read file" in snippet.content["result_text"]
            assert snippet.content["source"] == "LocalBookFile:error_book.txt"

    async def test_get_book_detail_match_file_not_found_on_disk_unexpected(self, client_with_mocks):
        client, _, _, _ = client_with_mocks
        ...
        with patch('builtins.open', mock_open()) as mocked_open_file:
            ...
            mocked_open_file.assert_called_once_with(os.path.join(TEST_BOOKS_DIR, "ghost_book.txt"), 'r', encoding='utf-8')
            assert "Error: Book file 'ghost_book.txt' was matched but could not be found on disk." in snippet.content["result_text"]
    ...
    async def test_get_book_detail_match_content_truncation(self, client_with_mocks):
        ...
        with patch('builtins.open', mock_open(read_data=long_content)) as mocked_open_file:
            snippet = await client._handle_get_book_detail_command(["long_story"])

            mocked_open_file.assert_called_once_with(os.path.join(TEST_BOOKS_DIR, "long_story.txt"), 'r', encoding='utf-8')
            assert f"Content for long_story.txt:\n\n{expected_truncated_content}" == snippet.content["result_text"]
